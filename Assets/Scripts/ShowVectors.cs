    ...
    void FixedUpdate()
    {
        ...
        if (showCollision)
        {
            if (_collisionForce != Vector3.zero)
            {
                ...
                float collisionScale = 10.0f * scaleFactor * (float)Math.Log(_collisionForce.magnitude+1);
        ...
        if (showGravity)
        {
            if (_gravityForce != Vector3.zero)
            {
                ...
                float gravityScale = 10.0f * scaleFactor * (float)Math.Log(_gravityForce.magnitude+1);
        ...
        if (showFriction)
        {
            if (_frictionForce != Vector3.zero)
            {
                ...
                float frictionScale = 10.0f * scaleFactor * (float)Math.Log(_frictionForce.magnitude+1);
        ...
        if (showNormal)
        {
            if (_normalForce != Vector3.zero)
            {
                ...
                float normalScale = 10.0f * scaleFactor * (float)Math.Log(_normalForce.magnitude+1);
        ...
        if (showAddedForce)
        {
            if (_addedForce != Vector3.zero)
            {
                ...
                float addedScale = 10.0f * scaleFactor * (float)Math.Log(_addedForce.magnitude+1);
